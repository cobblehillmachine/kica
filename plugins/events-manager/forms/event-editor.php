...
<form enctype='multipart/form-data' id="event-form" method="post" action="<?php echo esc_url(add_query_arg(array('success'=>null))); ?>">
	<div class="wrap">
		...
		<?php if(get_option('dbem_events_anonymous_submissions')): ?>
			<h2 class="event-form-submitter"><?php esc_html_e( 'Your Details', 'dbem' ); ?></h2>
			<div class="inside event-form-submitter your-details">
				<div>
					<h3><?php esc_html_e('Name *', 'dbem'); ?></h3>
					<input type="text" name="event_owner_name" id="event-owner-name" value="<?php echo esc_attr($EM_Event->event_owner_name); ?>" />
				</div>
				<div>
					<h3><?php esc_html_e('Email *', 'dbem'); ?></h3>
		...
		<?php endif; ?>
		<h2 class="event-form-submitter">Event Details</h2>
		...
		<div class="event-form-where">
			<h3><?php esc_html_e( 'Where *', 'dbem' ); ?></h3>
